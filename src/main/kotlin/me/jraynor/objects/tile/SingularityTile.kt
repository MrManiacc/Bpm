...
import me.jraynor.logic.select.SelectionHelper
 ...
 */
@Suppress("UNREACHABLE_CODE") class SingularityTile : GraphTileEntity(Registry.Tiles.SINGULARITY_TILE) {
    ...
    private fun processNodes() {
        nodeGraph.ifPresent { graph ->
            ...
            graph.forEach {
                if (it.graph.isEmpty()) it.graph = graph
                if (!world!!.isRemote) {
